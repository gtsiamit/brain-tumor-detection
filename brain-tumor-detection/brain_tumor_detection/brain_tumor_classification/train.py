...
def train():
    ...
    for train_index, test_index in skf.split(X, y):
        ...
        history = model.fit(
            X_train,
            tf.cast(y_enc_train, tf.float32),
            verbose=1,
            epochs=EPOCHS,
        ...
        plot_history(
            history=history,
            fullpath=RESULTS_PATH.joinpath(f"history_fold_{fold_num}.jpg"),
        )
        # store model locally
        save_model_locally(
            model=model, model_path=RESULTS_PATH.joinpath(f"model_fold_{fold_num}.h5")
        )
    ...
    plot_confusion_matrix(
        ...
        y_pred=le.inverse_transform(y_pred_all[:, 0].astype(int)),
        norm=False,
        fullpath=RESULTS_PATH.joinpath("cm.jpg"),
    )
    plot_confusion_matrix(
        y_true=le.inverse_transform(y_true_all),
        y_pred=le.inverse_transform(y_pred_all[:, 0].astype(int)),
        norm=True,
        fullpath=RESULTS_PATH.joinpath("normalized_cm.jpg"),
    )

    # store results
    save_pickle(
        fname=str(RESULTS_PATH.joinpath("y_pred_prob_all.pickle")),
        object=pd.DataFrame(y_pred_prob_all, columns=["pred_prob"]),
    )
    save_pickle(
        fname=str(RESULTS_PATH.joinpath("y_pred_all.pickle")),
        object=pd.DataFrame(y_pred_all, columns=["pred"]),
    )
    save_pickle(
        fname=str(RESULTS_PATH.joinpath("y_true_all.pickle")),
        object=pd.DataFrame(y_true_all, columns=["true"]),
    )
    save_pickle(
        fname=str(RESULTS_PATH.joinpath("fname_folds_all.pickle")),
        object=pd.DataFrame(fname_folds_all, columns=["fname"]),
    )
    save_pickle(
        fname=str(RESULTS_PATH.joinpath("fold_num_all.pickle")),
        object=pd.DataFrame(fold_num_all, columns=["fold"]),
    )
    ...
    if SAVE_FULL_MODEL:
        ...
        save_model_locally(
            model=model, model_path=RESULTS_PATH.joinpath("model_full.h5")
        )
...
def main():
    ...
    parser.add_argument(
        ...
        required=False,
        default="True",
        help="Train model on full dataset",
    )
